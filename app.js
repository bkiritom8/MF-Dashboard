...
function throttleDivDots(divs,startDate,endDate){
  ...
  if(perYear>52){
    var out=[],lastMs=-Infinity;
    divs.forEach(function(d){var ms=new Date(d.date).getTime();if(ms-lastMs>=7*864e5){out.push(d);lastMs=ms;}});
    return out;
  }
  if(perYear>12){
    var out=[],lastM='';
    divs.forEach(function(d){var m=d.date.slice(0,7);if(m!==lastM){out.push(d);lastM=m;}});
    return out;
  }
  return divs;
}
...
async function init(){
  ...
  renderFunds();updateSummary();loadTopFunds();
...
function renderFunds(){
  ...
  g.innerHTML=portfolio.map(function(f,i){
    var badge=isIDCW(f.name)?'<span class="idcw-badge">IDCW</span>':'<span class="growth-badge">Growth</span>';
    return '<div class="fund-card" id="fc-'+i+'">'+
      '<div class="fund-card-top">'+
        ...
        '<div class="fund-actions">'+
          ...
          '<button class="fund-btn" onclick="addToCompareFromPortfolio('+i+')">+ Compare</button>'+
...
async function loadFundData(f,i){
  ...
  var cur=nav*f.units,gain=cur-f.amount,ap=gain/f.amount;
  ...
  var totalRetPct=(cur+totalDiv-f.amount)/f.amount;
...
function setupCanvas(canvas){
  var W=canvas.offsetWidth||900,H=parseInt(canvas.getAttribute('height'))||420;
  canvas.width=W;canvas.height=H;
  return{W:W,H:H,w:W-PAD.l-PAD.r,h:H-PAD.t-PAD.b,ctx:canvas.getContext('2d')};
}

function drawGrid(ctx,W,H,w,h,minV,maxV){
  ...
  for(var g=0;g<=4;g++){
    ...
    ctx.fillText('Rs.'+val.toFixed(val<10?3:val<100?2:0),PAD.l-4,y+4);
  }
}

function drawXLabels(ctx,dates,H,w){
  ...
  for(var i=0;i<dates.length;i+=step){
    var x=PAD.l+(i/(dates.length-1))*w;
    ctx.fillText(dates[i].slice(0,7),x,H-PAD.b+16);
  }
}

function yFloor(minNav){
  if(minNav<50)return 0;
  if(minNav<100)return 50;
  if(minNav<500)return 100;
  if(minNav<1000)return 500;
  return Math.floor(minNav/500)*500;
}

function attachZoom(canvas,state,redrawFn){
  var dragStart=-1,dragging=false;
  function idxAt(e,s,e2){
    var rect=canvas.getBoundingClientRect();
    var cx=(e.touches?e.touches[0].clientX:e.clientX)-rect.left;
    var w=canvas.width-PAD.l-PAD.r;
    return Math.round(s+((cx-PAD.l)/w)*(e2-s));
  }
  canvas.addEventListener('mousedown',function(e){dragStart=idxAt(e,state.start,state.end);dragging=true;});
  canvas.addEventListener('mousemove',function(e){if(!dragging)return;var c=idxAt(e,state.start,state.end);redrawFn(Math.min(dragStart,c),Math.max(dragStart,c),true);});
  canvas.addEventListener('mouseup',function(e){
    if(!dragging)return;dragging=false;
    var c=idxAt(e,state.start,state.end);
    if(c<dragStart)redrawFn(state.fullStart,state.fullEnd,false);
    else if(c-dragStart>2)redrawFn(Math.min(dragStart,c),Math.max(dragStart,c),false);
  });
  canvas.addEventListener('dblclick',function(){redrawFn(state.fullStart,state.fullEnd,false);});
  canvas.addEventListener('mouseleave',function(){dragging=false;});
  canvas.addEventListener('touchstart',function(e){dragStart=idxAt(e,state.start,state.end);},{passive:true});
  canvas.addEventListener('touchend',function(e){
    var c=idxAt(e,state.start,state.end);
    if(c<dragStart)redrawFn(state.fullStart,state.fullEnd,false);
    else if(c-dragStart>2)redrawFn(dragStart,c,false);
  },{passive:true});
}

// ── NAV Chart ─────────────────────────────────────────────
var navState={idx:-1,start:0,end:0,fullStart:0,fullEnd:0};
...
function openChart(i){
  ...
  show('chart-modal');
  setTimeout(function(){
    var data=portfolio[i]._data;if(!data)return;
    navState.fullStart=0;navState.fullEnd=data._parsed.length-1;
    navState.start=0;navState.end=data._parsed.length-1;
    drawNavChart();
    attachZoom(document.getElementById('nav-chart'),navState,function(s,e,preview){
      navState.start=s;navState.end=e;if(!preview)drawNavChart();
    });
    setupNavTooltip();
  },60);
}
...
function setNavRange(days){
  ...
  var data=portfolio[navState.idx]._data;if(!data)return;
...
function drawNavChart(){
  ...
  var vis=pts.slice(navState.start,navState.end+1);
  if(!vis.length)return;
  var c=setupCanvas(document.getElementById('nav-chart'));
  var navs=vis.map(function(x){return x.nav;});
  var rawMin=Math.min.apply(null,navs),rawMax=Math.max.apply(null,navs);
  var minN=yFloor(rawMin),maxN=rawMax*1.03;

  var purchaseIdx=pts.findIndex(function(p){return p.date>=f.date;});
  var refNav=(purchaseIdx>=navState.start&&purchaseIdx<=navState.end)?pts[purchaseIdx].nav:vis[0].nav;
  ...
  drawXLabels(c.ctx,vis.map(function(x){return x.date;}),c.H,c.w);

  // Dividend dots
  var visDivs=(f._divs||[]).filter(function(d){return d.date>=vis[0].date&&d.date<=vis[vis.length-1].date;});
  throttleDivDots(visDivs,vis[0].date,vis[vis.length-1].date).forEach(function(d){
    var di=vis.findIndex(function(p){return p.date>=d.date;});if(di<0)return;
    c.ctx.beginPath();c.ctx.arc(xp(di),yp(vis[di].nav),5,0,2*Math.PI);
    c.ctx.fillStyle='#fbbf24';c.ctx.fill();c.ctx.strokeStyle='#0f172a';c.ctx.lineWidth=1.5;c.ctx.stroke();
  });
...
function setupNavTooltip(){
  ...
  canvas.onmousemove=function(e){
    ...
    var vis=f._data._parsed.slice(navState.start,navState.end+1);
    if(!vis.length)return;
    var w=canvas.width-PAD.l-PAD.r;
    var idx=Math.max(0,Math.min(vis.length-1,Math.round(((mx-PAD.l)/w)*(vis.length-1))));
    var pt=vis[idx];
    ...
    tooltip.style.left=Math.min(mx+12,canvas.width-200)+'px';
...
function renderDividendTable(f,el){
  var divs=f._divs||[];
  if(!isIDCW(f.name)){el.innerHTML='<div style="font-size:.78rem;color:#475569">Growth plan - returns compound in NAV, no dividend payouts.</div>';return;}
  if(!divs.length){el.innerHTML='<div style="font-size:.78rem;color:#475569">No dividend events detected since purchase date.</div>';return;}
  var totalDiv=divs.reduce(function(s,d){return s+d.total;},0);
  var rows=divs.slice().reverse().slice(0,15).map(function(d){
    return '<tr><td>'+fmtDate(d.date)+'</td><td class="positive">'+fmt2(d.divPerUnit)+'</td><td class="positive">'+fmt(d.total)+'</td></tr>';
  }).join('');
  el.innerHTML='<div style="font-size:.78rem;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:.5rem">Dividend history since purchase ('+f.units+' units)</div>'+
    ...
    '<div style="font-size:.72rem;color:#64748b;margin-top:.5rem">Showing last '+Math.min(divs.length,15)+' of '+divs.length+' payouts. Total: <span class="positive">'+fmt(totalDiv)+'</span></div>';
}

// ── Buy/Hold/Avoid Signal ─────────────────────────────────
function renderSignal(data,f,el){
  var arr=data._parsed;if(arr.length<60){el.innerHTML='';return;}
  var score=0,max=0,details=[];
  function item(label,val,good,bad,w){
    max+=w;if(val===null)return;
    score+=val>=good?w:val<=bad?0:Math.round(w*(val-bad)/(good-bad));
  ...
  max+=2;
  if(arr.length>=200){var ma=arr.slice(-200).reduce(function(s,x){return s+x.nav;},0)/200;var ab=today.nav>ma;score+=ab?2:0;details.push('Above 200MA: '+(ab?'Yes':'No'));}
  max+=2;
  if(arr.length>=63){
  ...
  if(arr.length>=126){
    var peak=0,dd=0;
    arr.slice(-126).forEach(function(x){if(x.nav>peak)peak=x.nav;var d=(peak-x.nav)/peak;if(d>dd)dd=d;});
...
async function addToCompareFromPortfolio(i){
  var f=portfolio[i];
  await addToCompare(f.code,f.name);
}

async function addToCompareFromCode(code,btn){
  var row=btn.closest('tr');
  var name=row?row.cells[1].textContent.trim():'Fund '+code;
  btn.textContent='Adding...';btn.disabled=true;
  await addToCompare(code,name);
...
function removeFromCompare(code){
  ...
  else{hide('compare-chart-wrap');document.getElementById('compare-table').classList.add('hidden');}
}
...
function setCompareRange(days){
  ...
  event.target.classList.add('active');
  var pts=compareState.pts;if(!pts||!pts.length)return;
  if(days===0){compareState.start=compareState.fullStart;compareState.end=compareState.fullEnd;}
    ...
    compareState.start=Math.max(0,si<0?0:si);compareState.end=compareState.fullEnd;
  }
  drawCompareChart();
}
...
async function buildCompareChart(){
  ...
  // Common window = latest start date to earliest end date
  ...
  allData.forEach(function(d){
    if(!d)return;
    d._parsed.forEach(function(p){if(p.date>=commonStart&&p.date<=commonEnd)dateSet[p.date]=true;});
  });
  var dates=Object.keys(dateSet).sort();
  if(dates.length<2){
    document.getElementById('compare-table').innerHTML='<p style="color:#f87171;padding:1rem">Funds have no overlapping date range.</p>';
    document.getElementById('compare-table').classList.remove('hidden');return;
  }

  compareState.pts=dates;compareState.fullStart=0;compareState.fullEnd=dates.length-1;
  compareState.start=0;compareState.end=dates.length-1;
  compareList.forEach(function(f,i){f._data=allData[i];});

  drawCompareChart();
  attachZoom(document.getElementById('compare-chart'),compareState,function(s,e,preview){
    compareState.start=s;compareState.end=e;if(!preview)drawCompareChart();
  });
...
function drawCompareChart(){
  ...
  var minV=yFloor(rawMin),maxV=rawMax*1.03;
  ...
  compareList.forEach(function(f,si){
    ...
    if(isIDCW(f.name)){
      ...
      throttleDivDots(visDivs,visDates[0],visDates[visDates.length-1]).forEach(function(dv){
        var di=visDates.findIndex(function(dt){return dt>=dv.date;});if(di<0)return;
        var nav=getNavOnDate(d,visDates[di]);
        c.ctx.beginPath();c.ctx.arc(xp(di),yp(nav),4,0,2*Math.PI);
        c.ctx.fillStyle=color;c.ctx.fill();c.ctx.strokeStyle='#0f172a';c.ctx.lineWidth=1.2;c.ctx.stroke();
      });
    }
  });

  drawXLabels(c.ctx,visDates,c.H,c.w);
  ...
  canvas.onmousemove=function(e2){
    ...
    var idx=Math.max(0,Math.min(visDates.length-1,Math.round(((mx-PAD.l)/c.w)*(visDates.length-1))));
    var date=visDates[idx];
    var lines=compareList.map(function(f,si){
      if(!f._data)return'';
      return '<span style="color:'+CHART_COLORS[si]+'">'+f.name.substring(0,22)+'...</span> Rs.'+getNavOnDate(f._data,date).toFixed(2);
    }).join('<br>');
    tooltip.innerHTML='<strong>'+date+'</strong><br>'+lines;
    tooltip.style.left=Math.min(mx+12,c.W-200)+'px';tooltip.style.top='8px';
    tooltip.classList.remove('hidden');
  };
  canvas.onmouseleave=function(){tooltip.classList.add('hidden');};

  document.getElementById('compare-legend').innerHTML=compareList.map(function(f,i){
    return '<div class="legend-item"><span class="legend-dot" style="background:'+CHART_COLORS[i]+'"></span>'+f.name.substring(0,50)+'</div>';
  }).join('');
}
...
async function buildCompareTable(allData,commonStart,commonEnd){
  ...
  var rows=await Promise.all(compareList.map(async function(f,si){
    ...
    var pts=d._parsed,nav=latestNAV(d);
    ...
    function retY(days){return pts.length<days?null:(nav-pts[pts.length-days].nav)/pts[pts.length-days].nav;}
    var navAtStart=getNavOnDate(d,commonStart);
    var totalReturn=(nav-navAtStart)/navAtStart+totalDivPerUnit/navAtStart;
    var sig=document.createElement('div');renderSignal(d,{name:f.name},sig);
    var st=sig.querySelector('.signal-title');
    return{name:f.name,color:CHART_COLORS[si],r1:retY(252),r3:retY(756),r5:retY(1825),
      totalReturn:totalReturn,divPerUnit:totalDivPerUnit,divCount:divs.length,
      signal:st?st.textContent.replace('Signal: ',''):'N/A'};
  }));
  var valid=rows.filter(Boolean);if(!valid.length)return;
  ...
  var html='<h4>Comparison — common window: '+commonStart+' to '+commonEnd+'</h4>'+
    '<table class="compare-table"><tr><th>Fund</th><th>Total Return*</th><th>1Y NAV</th><th>3Y NAV</th><th>5Y NAV</th><th>Div/Unit</th><th>Payouts</th><th>Signal</th></tr>';
  valid.forEach(function(d){
    html+='<tr>'+
      '<td><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:'+d.color+';margin-right:.4rem;vertical-align:middle"></span>'+d.name.substring(0,30)+'...</td>'+
  ...
  html+='</table><p style="font-size:.7rem;color:#475569;margin-top:.6rem">*Total return = NAV gain + dividends per unit from common start. Dots = dividend events. Not financial advice.</p>';
...
function searchFund(q){
  clearTimeout(searchTimer2);if(q.length<2){hide('fund-dropdown');return;}
...
function selectFund(code,name){selectedFund={code:code,name:String(name).trim()};document.getElementById('fund-search').value=selectedFund.name;hide('fund-dropdown');}
...
async function loadTopFunds(){
  ...
  var scored=[];
  for(var b=0;b<Math.min(results.length,30);b+=6){
    var batch=results.slice(b,b+6);
    var fetched=await Promise.all(batch.map(async function(c){
      ...
      var nav=latestNAV(data),old=getNavOnDate(data,daysAgoStr(period));
      if(!old||old<=0)return null;
      return{name:c.schemeName,code:c.schemeCode,ret:Math.pow(nav/old,365/period)-1,navNow:nav};
    }));
...
function renderTopTable(funds,cat,period){
  ...
  html+='<table class="top-table"><tr><th>#</th><th>Fund</th><th>Annualized Return</th><th>Current NAV</th><th></th></tr>';
  funds.forEach(function(f,i){
    var badge=i===0?'gold':i===1?'silver':i===2?'bronze':'';
    html+='<tr>'+
      '<td><span class="rank-badge '+badge+'">'+(i+1)+'</span></td>'+
      '<td style="max-width:260px;line-height:1.4">'+f.name+'</td>'+
      '<td class="'+cc(f.ret)+'">'+pct(f.ret)+'</td>'+
      '<td class="neutral">Rs.'+f.navNow.toFixed(2)+'</td>'+
      '<td><button class="fund-btn" onclick="addToCompareFromCode('+f.code+',this)">+ Compare</button></td>'+
    '</tr>';
  });
...
function importPortfolio(e){
  ...
  reader.onload=function(ev){
    try{
      ...
      savePortfolio(data);portfolio=data;renderFunds();updateSummary();alert('Imported successfully!');
